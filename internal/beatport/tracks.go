...
func (t *Track) Filename(n NamingPreferences) string {
	...
	if releaseDateParsed, err := time.Parse("2006-01-02", t.Release.Date); err == nil {
		releaseDay = releaseDateParsed.Format("02")
		releaseMonth = releaseDateParsed.Format("01")
		releaseYear = releaseDateParsed.Format("2006")
	}

	// Get current date for purchase/download dates
	now := time.Now()
	purchaseDay := now.Format("02")
	purchaseMonth := now.Format("01")
	purchaseYear := now.Format("2006")
	downloadDay := purchaseDay
	downloadMonth := purchaseMonth
	downloadYear := purchaseYear
