...
| `create_m3u8_playlist`        | false                                     | Boolean    | Create an m3u8 playlist file when multiple tracks are downloaded from a single URL                                                                                                        |
...
```shell
./beatportdl
Enter url, track ID, or search query:
```

### Download Methods

1. **URLs**: Enter a full Beatport or Beatsource URL
2. **Track IDs**: Enter just the numeric track ID (e.g., `1696999`)
   - When entering a track ID interactively, you'll be prompted to choose between Beatport and Beatsource
3. **Search**: Enter any search term to find tracks and releases
...
...or specify URLs/track IDs using positional arguments:

```shell
./beatportdl https://www.beatport.com/track/strobe/1696999 1696999 https://www.beatport.com/track/move-for-me/591753
```

For track IDs specified via command line, use the `-store` flag to choose the store:

```shell
./beatportdl -store beatport 1696999
...
# Track IDs (will use store specified by -store flag)
1696999
591753
8472639

# Mixed content is supported
https://www.beatsource.com/track/another-track/123456
```

Use the `-store` flag to specify which store to use for track IDs in text files:
...
```
-config-dir string
    Directory to load configuration files from (beatportdl-config.yml and beatportdl-credentials.json)
-q
